...
struct args_t {
    unsigned int local_array_size;
    unsigned int global_array_size;
    int local_num_ops;
    int global_num_ops;
    enum OpType op_type;
    int num_runs;
...
//    rofi_init("verbs", "mlx5_0");
    ...
    if (0 == me) {
      printf("Global Array Size   : %d\n", args.global_array_size);
      printf("Local Array Size    : %d (%zu B)\n", args.local_array_size, args.local_array_size * sizeof(unsigned long long));
      printf("Global Num Ops      : %d\n", args.global_num_ops);
      printf("Local Num Ops       : %d\n", args.local_num_ops);
      printf("World Size          : %d\n", np);
      fflush(stdout);
    }
