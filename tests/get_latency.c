// A simple one-sided get latency benchmark based on the OSU MPI one-sided get latency benchmark.
// This benchmark times the time to do rofi_get followed by rofi_wait.
//
...
int main(void) {
    ...
    if (0 == myrank) {
      ...
      printf("# msg_size_bytes, get_latency_usecs\n");
      fflush(stdout);
    }
    ...
    // rank 0 is the initiator (i.e., issues the get), and rank 1 is the target of the get
    ...
    for (cur_msg_size = MIN_MSG_SIZE; cur_msg_size <= MAX_MSG_SIZE; cur_msg_size = cur_msg_size * 2) {
      ...
      if (0 == myrank) {
        ...
        for (int i = 0; i < NUM_WARMUP_ITERS; i++) {
          rofi_get(buffer, buffer, cur_msg_size, 1, 0x0);
          rofi_wait();
        }
    ...
    printf("# Rank %d: Exiting get_latency benchmark\n", myrank);
    fflush(stdout);
